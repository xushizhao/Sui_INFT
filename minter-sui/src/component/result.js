    ...
    const self = {
        ...
        clickUnSell: (ev) => {
            ...
            Network("sui").load(fa, password, (pair) => {
                Network("sui").revoke(pair, name, (res) => {
        ...
        clickSell: (ev) => {
            ...
            Network("sui").load(fa, password, (pair) => {
                setPassword("");
                Network("sui").sell(pair, name, price, (res) => {
                    if (res.error) return setInfo(res.error);
                    setInfo(res.msg);
                    if (res.status === "Finalized") {
        ...
        show: () => {
            ...
            Network("sui").view(props.name, "selling", (dt) => {
                if (dt && dt.length === 3) {
    ...
    return (
        <Row>
            <Col className="pt-2" sm={size.back[0]} xs={size.back[0]}>
                {name.length>20?(tools.shorten(name,6)):name} at block {block.toLocaleString()}
            </Col>
