...
function Account(props) {
    ...
    const self = {
        ...
        clickNewAccount: (ev) => {
            setNewDisable(true);
            Network("sui").generate(password,(fa)=>{
        ...
        clickFaucet: async (ev) => {
            ...
            try {
                ...
                Network("sui").airdrop(login.address,(res)=>{
                    console.log(res);
                    if(!res.error){
                        self.showBalance(login.address);
                    }else{
                        setFaucet("Failed to get airdrop.");
                        return setTimeout(() => {
                            setFaucet("");
                        }, 3000);
                    }
            ...
            try {
                ...
                reader.onload = (e) => {
                    try {
                        ...
                        if (sign.address.length !== 66)
                            return setInfo("Error Sui address");
                        // if (sign.encoded.length !== 160)
        ...
        showBalance:(address)=>{
            //console.log(address);
            Network("sui").balance(address, (res) => {
                //console.log(res);
                //setBalance(res.totalBalance);
                const divide = Network("sui").divide();
                setBalance(tools.toF(res.totalBalance * (1 / divide), 8));
            })
        },
        show: () => {
            const fa = Local.get("login");
